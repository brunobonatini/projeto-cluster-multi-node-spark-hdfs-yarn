...
spark = SparkSession.builder.appName("projetovoos-exec").getOrCreate()
...
grid = grid.addGrid(modelo_rl.regParam, np.arange(0, .1, .01))
grid = grid.addGrid(modelo_rl.elasticNetParam, [0,1])

# Constrói o grid
grid = grid.build()

# Cria o CrossValidator
cv = tune.CrossValidator(estimator = modelo_rl,
...
best_lr.write().overwrite().save('/opt/spark/data/melhor_modelo_lr')
